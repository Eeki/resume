<script lang="ts">
  ...
  $: width = 0
  $: xMin = Math.min.apply(null, projects.map(({ start }) => start))
  $: xMax = Math.max.apply(null, projects.map(({ end }) => end))
  $: xScale = scaleTime().domain([xMin, xMax]).range([xMargin, width - xMargin]).nice()
...
<div class="relative w-full" bind:clientWidth={width}>
        ...
        {#if width}
            ...
            <g transform={`translate(${xMargin},0)`} >
                {#each projects as project, i}
                    <Project {project} {xScale} {setToolTip} y={i * projectHeight} chartWidth={width - xMargin * 2} />
                {/each}
            </g>
        {/if}
    </svg>
    ...
    <div class="absolute top-0 left-0 bottom-0 right-0 pointer-events-none">
        {#if toolTip}
            <ToolTip toolTip={toolTip} maxWidth={width} />
        {/if}
    </div>
</div>
